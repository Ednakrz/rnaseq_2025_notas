...
## *********** Código para crear las gráficas del ejercicio *******************
...
#
# plot.data$ColorBy <- colData(se)[, "layer_guess"];
#
# plot.data$ShapeBy <- colData(se)[, "layer_guess"];
#
...
#   labs(x="Dimension 1", y="Dimension 2", color="layer_guess", shape="layer_guess", title="PCA") +
#   coord_cartesian(xlim=range(plot.data$X, na.rm=TRUE),
#                   ylim=range(plot.data$Y, na.rm=TRUE), expand=TRUE) +
#   scale_color_manual(values=colDataColorMap(colormap, "layer_guess", discrete=TRUE)(7), na.value='grey50', drop=FALSE) +
#   scale_fill_manual(values=colDataColorMap(colormap, "layer_guess", discrete=TRUE)(7), na.value='grey50', drop=FALSE) +
...
# .column_data <- colData(se)[, "layer_guess", drop=FALSE]
# .column_data[["Selected points"]] <- iSEE::multiSelectionToFactor(list(), colnames(se))
#
# .column_col <- list()
#
# .color_values <- .column_data[["layer_guess"]]
# .color_values <- setdiff(unique(.color_values), NA)
# .col_colors <- colDataColorMap(colormap, "layer_guess", discrete=TRUE)(length(.color_values))
# if (is.null(names(.col_colors))) names(.col_colors) <- levels(factor(.color_values))
# .column_col[["layer_guess"]] <- .col_colors
#
...
# .column_annot_order <- order(.column_data[["Selected points"]], .column_data[["layer_guess"]])
